...
| `InstallSwiftDialog` | bool        | no       | If `true` and swiftDialog is missing from `/usr/local/bin/dialog`, download and install the latest release from GitHub before starting. Defaults to `false`. Prefer deploying swiftDialog as a managed package: this needs `api.github.com` to be reachable at enrollment time, which is often exactly when it is not (proxy, captive portal, or a VPN step that has not run yet). |
| `SwiftDialogTeamID` | string       | no       | Developer ID team identifier the downloaded swiftDialog package must be signed by. Defaults to `PWA5E9TQ59` (CSIRO, who publish swiftDialog). Only relevant when `InstallSwiftDialog` is `true`. Override this if you mirror your own rebuilt package; a package that is validly signed but by any other team is refused, because "signed by *someone*" is not a supply-chain control when the result is installed as root. |
